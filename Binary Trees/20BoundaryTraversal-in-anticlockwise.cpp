...
public:
    ...
    void addLeftBoundary(Node *root, vector<int> &res) // REVIEW -
    {
        Node *curr = root->left;
        while (curr)
        {
    ...
    void addRightBoundary(Node *root, vector<int> &res)
    {
        ...
        vector<int> temp; // REVIEW or use a stack simply
        while (curr)
        {
            // If the current node is not a leaf,
    ...
    vector<int> printBoundary(Node *root)
    {
        ...
        // add its value to the result
        if (!isLeaf(root)) // REVIEW
        {
            res.push_back(root->data);
        }

        // Add the left boundary, leaves,z
